...
void PreProcessing::SelectSegments(const std::vector<Hit*>& hits1,
                                   ...
                                   float d_max, bool single) 
{   
    for (size_t i = 0; i < hits1.size(); ++i) 
    {
        for (size_t j = ((single) ? i + 1UL : 0); j < hits2.size(); ++j) 
        {                        
            ...
            if((distance > 0.0) && (distance <= d_max))
            {               
                Edge edge;
    ...
    {        
        ...
        for (size_t i = 0; i < section_hits.size(); ++i) 
        {
            std::vector<float> pos = {section_hits[i].r / params.rmax,
                                      section_hits[i].phi / static_cast<float>(M_PI),
        ...
        for (int row = 0; row < static_cast<int>(params.num_rows); ++row) 
        {            
            std::vector<Hit*> hits1, hits2, hits3, hits4;
            ...
            if (row_groups.find(row + 1) != row_groups.end())
            {
            ...
            if (row_groups.find(row + 2) != row_groups.end())
            {
                for (size_t idx : row_groups[row + 2]) 
                {
                    hits3.push_back(&section_hits[idx]);
                }
            }

            if (row_groups.find(row + 3) != row_groups.end())
            {
                for (size_t idx : row_groups[row + 3]) 
                {
                    hits4.push_back(&section_hits[idx]);
                }
            }
            ...
            if(!hits1.empty())
            {
                SelectSegments(hits1, hits1, segments, params.d_max, true);

                if(!hits2.empty())
                {
                    SelectSegments(hits1, hits2, segments, params.d_max);
                }

                if(!hits3.empty())
                {
                    SelectSegments(hits1, hits3, segments, params.d_max);
                }

                if(!hits4.empty())
                {
                    SelectSegments(hits1, hits4, segments, params.d_max);
                }
            }
...
void PreProcessing::SaveGraphSample(const GraphSample& sample, const std::string& filename) 
{
    ...
    if (!std::filesystem::exists(dirPath)) 
    {
        ...
        {
            throw std::runtime_error("SaveGraphSample: Fail to create the folder " + dirPath.string());
        }
    }
