...
int main(int argc, char* argv[]) 
{
    ...
    float true_positive[num_points]{0.0};
    float true_negative[num_points]{0.0};
    float false_positive[num_points]{0.0};
    float false_negative[num_points]{0.0};
    ...
    for (auto batch : test_dataloader) 
    {
        ...
        {            
            ...
            true_positive[i]  += ((pred_labels == 1) * (true_labels == 1)).sum().item<float>();
            true_negative[i]  += ((pred_labels == 0) * (true_labels == 0)).sum().item<float>();
            false_positive[i] += ((pred_labels == 1) * (true_labels == 0)).sum().item<float>();
            false_negative[i] += ((pred_labels == 0) * (true_labels == 1)).sum().item<float>();
        }
    }

    for (int i = 0; i < num_points; ++i)
    {
        accuracy_array[i] = (true_positive[i] + true_negative[i]) / (true_positive[i] + true_negative[i] + false_positive[i] + false_negative[i]);
        purity_array[i] = (true_positive[i] + false_positive[i]) > 0 ? true_positive[i] / (true_positive[i] + false_positive[i]) : 0.0;
        efficiency_array[i] = (true_positive[i] + false_negative[i]) > 0 ? true_positive[i] / (true_positive[i] + false_negative[i]) : 0.0;
    }
